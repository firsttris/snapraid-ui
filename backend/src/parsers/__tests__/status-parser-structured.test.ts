...
Deno.test("parseStatusOutput - parses snapraid structured status output correctly", async () => {
  ...
  assertEquals(status.wastedGB, 499.8);
  assertEquals(status.freeSpaceGB, 209.3);
  assertEquals(status.totalFiles, 22);
  assertEquals(status.totalUsedGB, 0.1);
  assertEquals(status.totalFreeGB, 209.3);
  ...
  assertEquals(status.disks![0].usedGB, 0.1);
  assertEquals(status.disks![0].freeGB, 209.3);
